...
//  k (5) Create static constant integer values named CODE_WEATHER & CODE_WEATHER_WITH_DATE to identify the URIs this ContentProvider can handle
    public static final int CODE_WEATHER = 200;
    public static final int CODE_WEATHER_WITH_DATE = 201;

//  k (7) Instantiate a static UriMatcher using the buildUriMatcher method
    public static final UriMatcher uriMatcher = buildUriMatcher();

    WeatherDbHelper mOpenHelper;

//  k (6) Write a method called buildUriMatcher where you match URI's to their numeric ID
    static UriMatcher buildUriMatcher() {
        UriMatcher matcher = new UriMatcher(UriMatcher.NO_MATCH);
        String auth = WeatherContract.CONTENT_AUTHORITY;
        String path = WeatherContract.PATH_WEATHER;
        matcher.addURI(auth, path, CODE_WEATHER);
        matcher.addURI(auth, path + "/#", CODE_WEATHER_WITH_DATE);
        return matcher;
    }

//  k (1) Implement onCreate
    @Override
    public boolean onCreate() {
//      k (2) Within onCreate, instantiate our mOpenHelper
        mOpenHelper = new WeatherDbHelper(getContext());

//      k (3) Return true from onCreate to signify success performing setup
        return true;
    }
...
//  k (8) Provide an implementation for the query method
    /**
    ...
    public Cursor query(@NonNull Uri uri, String[] projection, String selection,
        ...
        Cursor cursor;
//      k (9) Handle queries on both the weather and weather with date URI
        ...
        } else if(uriMatcher.match(uri) == CODE_WEATHER_WITH_DATE) {
...
//      k (10) Call setNotificationUri on the cursor and then return the cursor
