...
//  k (1) Implement the bulkInsert method
    /**
...
//          k (2) Only perform our implementation of bulkInsert if the URI matches the CODE_WEATHER code
        if (sUriMatcher.match(uri) == CODE_WEATHER) {
            database.beginTransaction();
            int rowsInserted = 0;
            try {
                for (ContentValues val : values) {
                    long _id = database.insert(WeatherContract.WeatherEntry.TABLE_NAME, null, val);
                    if (_id != -1) {
                        rowsInserted++;
                    }
                }
                ...
                throw new UnsupportedOperationException("Whoa can't insert that");
            }
...
//              k (3) Return the number of rows inserted from our implementation of bulkInsert

//          k (4) If the URI does match match CODE_WEATHER, return the super implementation of bulkInsert
    ...
    public int delete(@NonNull Uri uri, String selection, String[] selectionArgs) {
        ...
        if (sUriMatcher.match(uri) == CODE_WEATHER) {
            rowsDel = mOpenHelper.getWritableDatabase().delete(WeatherContract.WeatherEntry.TABLE_NAME, selection, selectionArgs);
        } else {
